...
export function LogtoEntityByIdPipe<T>(
  entityType: LogtoEntityType,
  ...
  class MixinLogtoEntityByIdPipe implements PipeTransform<string, Promise<T>> {
    ...
    async transform(id: string): Promise<T> {
      ...
      try {
        const fetchMethod = this.logtoRequests[config.method] as (
          id: string,
        ) => Promise<T>;
...
export const LogtoOrganizationByIdPipe = LogtoEntityByIdPipe(
  LogtoEntityType.ORGANIZATION,
);
export const LogtoInvitationByIdPipe = LogtoEntityByIdPipe(
  LogtoEntityType.INVITATION,
);
