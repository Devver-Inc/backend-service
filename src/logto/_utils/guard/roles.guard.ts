...
import { UserPermissionsEnum, UserRoleEnum, UserRoles } from '../enums/permissions.enum'
...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    const opts = this.reflector.getAllAndOverride<ProtectOptions>(ROLES_KEY, [context.getHandler(), context.getClass()])
    ...
    const requiredRoles = opts.roles && opts.roles.length > 0 ? new Set(opts.roles) : null
    const requiredPermissions = opts.permissions && opts.permissions.length > 0 ? new Set(opts.permissions) : null

    if (requiredRoles && userRoles.some((role) => requiredRoles.has(role.roleName as UserRoleEnum))) return true

    if (requiredPermissions && userPermissions.some((perm) => requiredPermissions.has(perm as UserPermissionsEnum)))
      return true

    if (requiredPermissions) {
      for (const role of userRoles) {
        const roleConfig = UserRoles[role.roleName as UserRoleEnum]
        if (roleConfig?.permissions?.some((perm: UserPermissionsEnum) => requiredPermissions.has(perm))) {
          return true
        }
      }
    }
