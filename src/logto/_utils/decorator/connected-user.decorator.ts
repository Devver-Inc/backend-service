...
import { LogtoOrganization, LogtoUser } from '../types/responses/responses.type'
import { LogtoUserWithOrganizations } from '../types/user-with-organization.type'

export const ConnectedUser = createParamDecorator((_, ctx: ExecutionContext): LogtoUser => {
...
export const ConnectedUserOrganizationOrNull = createParamDecorator(
  (_, ctx: ExecutionContext): LogtoOrganization | null => ctx.switchToHttp().getRequest().auth.organization,
)

export const ConnectedUserWithOrgs = createParamDecorator((_, ctx: ExecutionContext): LogtoUserWithOrganizations => {
