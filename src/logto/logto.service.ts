...
export class LogtoService {
  ...
  getUserInformations = (id: string) =>
    this.logtoRequests.fetchUserInformations(id);
  getOrganizationInformations = (id: string) =>
    this.logtoRequests.fetchOrganizationInformations(id);
  ...
  extractBearerTokenFromHeaders({
    ...
    if (!authorization.startsWith(BEARER_PREFIX)) {
      throw new AuthorizationError(
        `Authorization header must start with "${BEARER_PREFIX}"`,
        401,
      );
    }
  ...
  async manageUserWithoutOrganization(user: LogtoUser) {
    ...
    if (professionalDomain) {
      ...
      if (professionalOrganization)
        await this.logtoRequests.addOrganizationJitEmailDomain(
          professionalOrganization.id,
          professionalDomain,
        );
    }
  }
  ...
  async createOrganizationIfNotExistsAndAssignUserWithRoles(
    ...
    await this.logtoRequests.addUsersToOrganization(newOrganization.id, [
      user.id,
    ]);
  ...
  private async getRoleIdsForRoleNames(
    ...roleNames: LogtoRolesEnum[]
  ): Promise<string[]> {
  ...
  async updateUserPassword(
    ...
    if (user.hasPassword && updateUserPasswordDto.oldPassword) {
      const isPasswordValid = await this.logtoRequests.verifyUserPassword(
        user.id,
    ...
    return this.logtoRequests.updateUserPassword(
      user.id,
