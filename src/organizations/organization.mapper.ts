...
import { EnvironmentVariables, ServerConfig } from 'src/_utils/config/env.config'
...
export class OrganizationsMapper {
  ...
  toOrganizationLightDto = async (organization: LogtoOrganization): Promise<GetOrganizationLightDto> => ({
    id: organization.id,
  ...
  toOrganizationDetailsDto = async (
    ...
    admins: admins?.length ? admins.map((member) => this.usersMapper.toUserLightDto(member)) : [],
    membersCount: members.length,
  })

  toInvitationDtoFromArray = (invitations: LogtoInvitation[]): GetInvitationDto[] =>
    invitations.map((inv) => this.toInvitationDto(inv))

  toInvitationDto = (invitation: LogtoInvitation, organizationName?: string): GetInvitationDto => ({
    id: invitation.id,
    ...
    acceptedAt: invitation.acceptedAt ? new Date(invitation.acceptedAt).toISOString() : undefined,
  })

  toGetPermissions = (permissions: LogtoOrganizationPermissions): string[] => permissions.map((x) => x.name)

  toGetOrganizationMembersQuery = (paginatedQueryDto: UsersPaginatedQueryDto) => ({
    page: paginatedQueryDto.page,
    page_size: paginatedQueryDto.pageSize,
    ...(paginatedQueryDto.search && { q: paginatedQueryDto.search }),
  })

  toOrganizationCustomDataType = (ownerId: string, adminIds: string[]): OrganizationCustomData => ({
    ownerId: ownerId,
    adminIds: [ownerId, ...adminIds],
  })
  toGetInvitationLinkUrl = (invitation: LogtoInvitation, organizationName: string) => {
    const roles = invitation.organizationRoles?.map((role) => role.name).join(',') || ''
